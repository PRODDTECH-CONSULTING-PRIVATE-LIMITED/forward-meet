...
const App = () => {
  ...
  const [itemsPerPage, setItemsPerPage] = useState(5); // Made this configurable
  ...
  useEffect(() => {
    if (mapsLoaded && window.google?.maps?.places) {
      console.log("Google Maps Places library available. Initializing autocomplete.");
      if (userLocation) {
  ...
  useEffect(() => {
    if (map && midwayRestaurants.length > 0 && directionsService && directionsRenderer) {
      markers.forEach((marker) => marker.setMap(null));
  ...
  const handleNextImage = (placeIndex) => {
    ...
    const maxIndex = place.photo_references ? place.photo_references.length - 1 : 0;
    setCurrentImageIndex((prev) => ({
      ...prev,
  ...
  const handleSearch = async (e) => {
    ...
    try {
      const response = await fetch("http://localhost:8080/api/find_midway_restaurant", {
        method: "POST",
        headers: {
      ...
      if (err instanceof TypeError && err.message === "Failed to fetch") {
        setError("Failed to connect to the server. Please ensure the backend is running and accessible at http://localhost:8080.");
      } else {
        setError(`An unexpected error occurred: ${err.message}. Please check console for details.`);
      }
    } finally {
      setLoading(false);
    }
  ...
  const generateInvitation = async (place) => {
    ...
    try {
      ...
      const response = await fetch("http://localhost:8080/api/generate_invitation", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
      ...
      if (response.ok) {
        ...
        setInvitationError(data.error || `Failed to generate invitation: ${response.status} ${response.statusText}`);
      }
    } catch (err) {
      console.error("Fetch error for invitation generation:", err);
      setInvitationError("Failed to connect to the LLM server. Please ensure the backend is running.");
    } finally {
      setGeneratingInvitation(false);
    }
  };
  ...
  const currentItems = midwayRestaurants.slice(indexOfFirstItem, indexOfLastItem);
    ...
    return (
      <div className="flex items-center justify-center min-h-screen bg-gradient-to-br from-blue-100 to-purple-100">
        <div className="text-center">
          ...
          <p className="text-sm text-gray-500 mt-2">Please wait while we initialize Google Maps</p>
        </div>
      </div>
    );
  }
...
{/* LEFT PANEL */}
<div className="w-1/5 min-w-[280px] h-screen bg-white shadow-xl overflow-y-auto border-r">        <div className="p-6">
          {/* Header */}
          ...
          {showGeolocationPrompt && (
            <div className="mb-6 p-4 bg-amber-50 border-l-4 border-amber-400 rounded-r-lg" role="alert">
              <div className="flex items-start">
                ...
                <div>
                  <p className="font-semibold text-amber-800 text-sm mb-1">Location Access Recommended</p>
          ...
          <form onSubmit={handleSearch} className="space-y-4">
            ...
            <div className="space-y-4">
              <div>
                <label htmlFor="location1" className="block text-gray-700 text-sm font-semibold mb-2">
              ...
              <div>
                <label htmlFor="location2" className="block text-gray-700 text-sm font-semibold mb-2">
                  <span className="flex items-center">
            ...
            <div>
              <label htmlFor="placeType" className="block text-gray-700 text-sm font-semibold mb-2">
                <span className="flex items-center">
            ...
            <div className="bg-gray-50 p-4 rounded-xl border">
              ...
              <div className="flex gap-6">
                <label className="inline-flex items-center cursor-pointer">
                  ...
                  />
                  <span className="ml-2 text-gray-700 text-sm font-medium">⏱️ Time</span>
                </label>
                <label className="inline-flex items-center cursor-pointer">
                  <input
                    type="radio"
                    ...
                    onChange={(e) => setSearchMode(e.target.value)}
                  />
                  <span className="ml-2 text-gray-700 text-sm font-medium">📏 Distance</span>
                </label>
              </div>
            </div>
            ...
            <div className="bg-blue-50 p-4 rounded-xl border border-blue-100">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label htmlFor="selectedDate" className="block text-gray-600 text-xs font-semibold mb-1">
                    Date:
                  </label>
                  <input
                    type="date"
                ...
                <div>
                  <label htmlFor="selectedTime" className="block text-gray-600 text-xs font-semibold mb-1">
                    Time:
                  </label>
                  <input
                    type="time"
            ...
            {/* Search Radius */}
            <div>
              <label htmlFor="searchRadius" className="block text-gray-700 text-sm font-semibold mb-3">
                <span className="flex items-center justify-between">
                  <span>🎯 Search Radius</span>
                  <span className="text-green-600 font-bold">{searchRadius} km</span>
                </span>
              </label>
              <input
                type="range"
                ...
                style={{
                  background: `linear-gradient(to right, #10b981 0%, #10b981 ${(searchRadius / 50) * 100}%, #e5e7eb ${(searchRadius / 50) * 100}%, #e5e7eb 100%)`
                }}
              />
          ...
          {error && (
            <div className="mt-6 p-4 bg-red-50 border-l-4 border-red-400 rounded-r-lg" role="alert">
              <div className="flex items-start">
                <svg className="w-5 h-5 text-red-400 mt-0.5 mr-3 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
                  <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zM8.707 7.293a1 1 0 00-1.414 1.414L8.586 10l-1.293 1.293a1 1 0 101.414 1.414L10 11.414l1.293 1.293a1 1 0 001.414-1.414L11.414 10l1.293-1.293a1 1 0 00-1.414-1.414L10 8.586 8.707 7.293z" clipRule="evenodd" />
                </svg>
                <div>
                  <p className="font-semibold text-red-800 text-sm mb-1">Error occurred</p>
          ...
          {midwayRestaurants.length > 0 && (
            <>
              {/* Results Header */}
              <div className="mt-8 mb-4 p-4 bg-green-50 border-l-4 border-green-400 rounded-r-lg">
                  ...
                  <div className="flex items-center">
                    ...
                    <div>
                      ...
                      <p className="text-green-700 text-xs">
                        Showing {indexOfFirstItem + 1} to {Math.min(indexOfLastItem, midwayRestaurants.length)} of {midwayRestaurants.length}
                      </p>
                    </div>
                  </div>
              ...
              <div className="space-y-4">
                {currentItems.map((location, index) => (
                  <div key={location.place_id} className="bg-white rounded-lg shadow-md border border-gray-200 overflow-hidden hover:shadow-lg transition-shadow duration-200">
                    {/* <div className=""> */}
                      <div className="flex items-center justify-between mb-3">
                        {/* <div className="flex items-center space-x-3">
                          ...
                          <span className="text-sm font-semibold text-gray-700">
                            Result #{indexOfFirstItem + index + 1}
                          </span>
                        </div> */}
                        
                        {/* {location.travel_time_from_loc1_min && location.travel_time_from_loc2_min && (
                          <div className="flex space-x-2 text-xs">
                            <span className="bg-blue-100 text-blue-700 px-2 py-1 rounded-full">
                              {location.travel_time_from_loc1_min}min
                            </span>
                            <span className="bg-purple-100 text-purple-700 px-2 py-1 rounded-full">
                              {location.travel_time_from_loc2_min}min
                            </span>
                          </div>
                        )} */}
                      </div>
                      <GooglePlaceCard placeId={location.place_id} locaitonInfo={location}/>
                    {/* </div> */}
                  </div>
                ))}
              </div>
          ...
          {invitationDraft && (
            <div className="mt-6 p-4 bg-blue-50 border-l-4 border-blue-400 rounded-r-lg">
              ...
              <div className="bg-white rounded-lg p-3 shadow-inner border border-blue-100">
                <p className="text-gray-700 text-sm whitespace-pre-wrap">{invitationDraft}</p>
              </div>
            </div>
          )}

          {invitationError && (
            <div className="mt-4 p-3 bg-red-50 border-l-4 border-red-400 rounded-r-lg" role="alert">
              <p className="font-semibold text-red-800 text-sm mb-1">Invitation Error</p>
      ...
      {/* RIGHT PANEL: Google Map - FIXED HEIGHT AND ALWAYS VISIBLE */}
