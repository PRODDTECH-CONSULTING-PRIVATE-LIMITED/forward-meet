...
const AppGroup = (props) => {
  ...
  const [locations, setLocations] = useState(() => {
    try {
      ...
      // Group search path: locations array in state or URL
      if (stateLocations) return typeof stateLocations === 'string' ? JSON.parse(stateLocations) : stateLocations;
      if (urlLocations) return JSON.parse(urlLocations);
      // 2-person fallback: loc1 + loc2
      const loc1 = location.state?.loc1 || params.get('loc1');
      const loc2 = location.state?.loc2 || params.get('loc2');
...
const initMap = async () => {
  ...
  useEffect(() => {
    ...
    } else if (!isDetailedView && (midwayRestaurants.length > 0 || (locations && locations.length >= 2))) {
  ...
  }, [isDetailedView, detailedPlaceId, midwayRestaurants, locations, map, directionsService, directionsRenderer, userLocation]);
  ...
  const generateInvitation = async (place) => {
    ...
    try {
      const payload = {
        ...
        location1_name: locations && locations.length > 0 ? locations[0] : "",
        location2_name: locations && locations.length > 1 ? locations[1] : "",
      };
      ...
      <div className="h-screen bg-white shadow-2xl overflow-y-auto border-r border-slate-100 relative z-10">
        <div className="p-8 w-full" style={{paddingTop: "0px"}}>
          ...
          {showFilters && (
            ...
            >
                {/* Group Search Active - Inputs are on Dashboard */}
                <div className="p-4 bg-blue-50 rounded-lg text-blue-700 text-sm mb-4">
                  Group search active for {locations ? locations.length : 0} people.
                </div>
                <TrafficPrediction 
                  {...props} 
